...
class OSADLRefresher:
    """Download the latest OSADL matrix + copyleft data into the user cache."""
    ...
    def refresh(self, console: Console) -> None:
        """Download both OSADL files and prime the data store."""
    ...
    def download(self, url: str, dest: Path) -> None:
        """Download ``url`` to ``dest``, validating size and JSON payload."""
        ...
        # Validate JSON before writing so bad data can't land in the cache.
        json.loads(data)
